...
def train(ep, dataload):
    ...
    for cnn_traces, dnn_feats, ages, weights in dataload:
        traces = cnn_traces.transpose(1, 2)
        traces, ages, weights = traces.to(device), ages.to(device), weights.to(device)
        dnn_feats = dnn_feats.to(device)
        # Reinitialize grad
        model.zero_grad()
        # Send to device
        # Forward pass
        pred_ages = model(traces, dnn_feats)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--age_col', default='Age in months',
    ...
    parser.add_argument('--path_to_traces', default='D:\\浏览器下载\\agedata1\\agedata1',
        help='path to file containing ECG traces')
    parser.add_argument('--path_to_csv', default="age_regression.csv",
        help='path to csv file containing attributes.')
    parser.add_argument('--dnn_output_dim', default=512,
        help='dnn output size.')
    parser.add_argument('--feats_index', default=[
        'PR interval(ms)', 'P wave axis(degrees)', 'P wave width(ms)', 'QRS axis(degrees)', 'QRS width(ms)',
        'QTc interval(ms)', 'QT interval(ms)', 'R wave axis(degrees)', 'RV1(mV)', 'RV3', 'RV5(mV)',
        'SV1(mV)',
        'SV3', 'SV5(mV)', 'T wave axis(degrees)', 'T wave width(ms)', 'XP', 'HeartRate',
        'AtrialRate(min/time)',
        'VentricularRate(min/time)', 'score'],
        help='dnn feats index.')
    ...
    # Dataloader
    train_loader = BatchDataloader(traces, dnn_feats, ages, weights, bs=args.batch_size, mask=train_mask)
    valid_loader = BatchDataloader(traces, dnn_feats, ages, weights, bs=args.batch_size, mask=valid_mask)
    ...
    model = Branch_NN(
        dnn_input_dim=len(args.feats_index),
        dnn_output_dim=args.dnn_output_dim,
        cnn_input_dim=(N_LEADS, args.seq_length),
    ...
    for ep in range(start_epoch, args.epochs):
        ...
        # Save history
        # 创建一个新的行数据
        new_row = pd.DataFrame({"epoch":      [ep], "train_loss": [train_loss],
                                "valid_loss": [valid_loss], "lr": [learning_rate]})
        # 使用 pd.concat() 方法将新行追加到 DataFrame 中
